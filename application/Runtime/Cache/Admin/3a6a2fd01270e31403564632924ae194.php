	...
	<table>
        ...
        <tbody>
		 <?php if(is_array($doc_list)): foreach($doc_list as $key=>$vo): ?><tr>
				...
				<!--<td class="file">
					&lt;!&ndash;<a href="<?php echo U('downLoad','id='.$vo['doc_id']);?>">附件下载</a>&ndash;&gt;
					<a href="<?php echo U('downLoad','id='.$vo[doc_id]);?>">附件下载</a>
				</td>-->
				<?php if($vo["doc_file"] == './Upload/' ): ?><td class="file">
						无附件
					</td>
					<?php else: ?>
					<td class="file">
						<a href="/Admin/Doc/down/id/<?php echo ($vo["doc_id"]); ?>">附件下载</a>
					</td><?php endif; ?>
